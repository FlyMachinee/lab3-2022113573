...
public class Graph { // 图类
    ...
    // 添加一条从 fromWord 到 toWord 的有向边
    // 如果边已存在，则权重增加1 (表示相邻出现次数增加) [cite: 7]
    ...
    public Map<String, Integer> getSuccessorsWithWeights(String word) {
        if (!hasNode(word)) return Collections.emptyMap(); // 如果节点不存在，返回空映射
    ...
    // 获取两个单词之间的所有桥接词 [cite: 10]
    // 桥接词 word3 定义为存在有向边 word1 -> word3 和 word3 -> word2
    public List<String> getBridgeWords(String word1, String word2) {
        ...
        for (int bridgeId : successorsOfWord1.keySet()) {
            Map<Integer, Integer> successorsOfBridge = getSuccessorIdsWithWeights(bridgeId); // 获取该潜在桥接词的后继节点
    ...
    // 计算从 word1 到 word2 的一条最短路径及其长度 (使用Dijkstra算法) [cite: 16]
    public Map<String, Object> getShortestPath(String word1, String word2) {
        ...
        // 初始化所有距离为无穷大
        for (int i = 0; i < nextNodeId; i++) distances.put(i, Integer.MAX_VALUE);
        ...
        while (!pq.isEmpty()) { // 当优先队列不为空
            ...
            if (current.distance > distances.get(u)) continue; // 如果是已处理过的更长路径，跳过
            if (u == endId) { // 如果已到达目标节点
                List<String> stringPath = new ArrayList<>(); // 构建字符串表示的路径
                for (int nodeId : current.pathNodes) stringPath.add(getWordById(nodeId));
    ...
    // 计算图中所有节点的PageRank值 [cite: 17]
    // d: 阻尼系数 (通常为0.85)
    // iterations: 迭代次数
    public Map<String, Double> calculatePageRank(double d, int iterations) {
        int numNodes = getNodeCount(); //图中节点总数
        if (numNodes == 0) return Collections.emptyMap(); //空图则返回空

        Map<Integer, Double> pr = new HashMap<>(); //存储每个节点的PR值
        // 初始化：所有节点的PR值均设为 1/N [cite: 17]
        for (int i = 0; i < numNodes; i++) pr.put(i, 1.0 / numNodes);

        Map<Integer, Integer> outDegree = new HashMap<>(); //存储每个节点的出度
        // 计算每个节点的出度 L(v) [cite: 17]
        for (int i = 0; i < numNodes; i++) outDegree.put(i, getSuccessorIdsWithWeights(i).size());

        // 进行指定次数的迭代
        for (int iter = 0; iter < iterations; iter++) {
            Map<Integer, Double> newPr = new HashMap<>(); //存储本次迭代计算出的新PR值
            double danglingSum = 0.0; //用于累加悬挂节点(出度为0的节点)的PR值 [cite: 17]
            // 找出所有悬挂节点，并累加它们的PR值
            for (int i = 0; i < numNodes; i++) {
                if (outDegree.get(i) == 0) danglingSum += pr.get(i);
            }
